 ...
 */
class Service_Security extends Service {
     ...
     *
     * @param   boolean    $bo_input        The value to check
     ...
     *
     * @param   int     $i_input        The value to check
     * @param   boolean $bo_positive    Set to true for positive values only
     ...
     *
     * @param   float   $fl_input       The value to check
     * @param   boolean $bo_positive    Set to true for positive values only
    ...
    public function secureStringDB($s_input){
        if( is_null($this->service_Database) ){
     ...
     * 
     * @param   string  $s_value    The postal address
     * @return  boolean True if the postal address is valid, otherwise false
     */
    public function checkPostalNL($s_value){
        if( trim($s_value) == "" )   return false;
        if( !preg_match("/^\d{4}\s?[a-z]{2}$/i",$s_value) )  return false;
    ...
    public function checkPostalBE($i_value){
        if( trim($i_value) == "" )   return false;
        if( !preg_match("/^\d{4}$/",$i_value) ) return false;
    
        if( $i_value < 1000 || $i_value > 9999 )    return false;
    ...
    /**
     *
     * @param   string  $s_type      Type of input   (GET|POST|REQUEST)
     * @param   array   $a_declared  De type declare array (init_get,init_post,init_request)
     ...
     * 
     * @param   string  $s_type     The data type
     * @param   array   $a_source   The source array
     * @return  array   The secured input data
     */
    private function parseArray($s_type, $a_source) {
        ...
        foreach ($a_keys AS $s_key) {
            ...
            else if ($s_type == 'string-DB-array') {
                $a_result[$s_key] = $this->secureStringDB($a_source[$s_key]);
            }
            else if( $s_type == 'string-DB-JS-array' ){
		$a_result[$s_key] = $this->prepareJsDecoding($a_source[$s_key]);
		$a_result[$s_key] = $this->secureStringDB($a_result[$s_key]);
		$a_result[$s_key] = $this->fixDecodeBug($a_result[$s_key]);
            }
            else if ($s_type == 'string-array') {
                $a_result[$s_key] = $this->secureString($a_source[$s_key]);
            }
            else if( $s_type == 'string-JS-array' ){
		$a_result[$s_key] = $this->prepareJsDecoding($a_source[$s_key]);
		$a_result[$s_key] = $this->secureString($a_result[$s_key]);
		$a_result[$s_key] = $this->fixDecodeBug($a_result[$s_key]);
            }
            else if( $s_type == 'int-array' ){
                $a_result[$s_key]   = $this->secureInt($a_source[$s_key]);
            }
            else if( $s_type == 'float-array' ){
               $a_result[$s_key]   = $this->secureFloat($a_source[$s_key]); 
            }
        }
    ...
    public function prepareJsDecoding($s_value){
    	$s_value	= urldecode($s_value);
	$s_value	= str_replace(array('‘','’'), array('&lsquo;','&rsquo;'),$s_value);
     ...
     * 
     * @param string $s_text	The input text
     * @return string	The correct decoded text
     */
    public function fixDecodeBug($s_text){
    	$s_text	= preg_replace('/&acirc;{1}.+&not;{1}/','&euro;', $s_text);
    	
    	return str_replace(
    		array('&Atilde;&laquo;','&Atilde;&curren;','&Atilde;&yen;','&Atilde;&sect;','&Atilde;&uml;','&Atilde;&copy;',
    			'&Atilde;&macr;','&Atilde;&sup2;','&Atilde;&sup3;','&Atilde;&para;','&Acirc;&copy;','&amp;rsquo;','&amp;lsquo;'),
    		array('&euml;','&auml;','&aring;','&ccedil;','&egrave;','&eacute;','&iuml;','&ograve;','&oacute;','&ouml;',
    			'&copy;','&rsquo;','&lsquo;'),
    		$s_text);
    }
}
