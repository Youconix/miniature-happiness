...
class Model_Stats extends Model {
	...
	public function saveIP($s_ip,$s_page){
		Memory::type('string',$s_ip);
		Memory::type('string',$s_page);
		...
		$this->service_QueryBuilder->select('stats_unique','id')->getWhere()->addAnd(array('ip','datetime'),array('s','i','i'),
			array($s_ip,$i_begin,$i_end),array('=','BETWEEN'));
	...
	public function saveOS($s_os,$s_osType){
		Memory::type('string',$s_os);
		Memory::type('string',$s_osType);

		$this->service_QueryBuilder->update('stats_OS','amount','l','amount + 1')->getWhere()->addAnd(array('datetime','name','type'),
			array('i','s','s'),array($this->i_date,$s_os,$s_osType));
		$service_Database = $this->service_QueryBuilder->getResult();
			
		if( $service_Database->affected_rows() == 0 ){
			$this->service_QueryBuilder->insert('stats_OS',array('name','amount','datetime','type'),array('s','i' ,'i','s'),
				array($s_os,1,$this->i_date,$s_osType))->getResult();
		}
	}
	...
	public function saveBrowser($s_browser,$s_version){
		Memory::type('string',$s_browser);
		Memory::type('string',$s_version);

		$this->service_QueryBuilder->update('stats_browser','amount','l','amount + 1')->getWhere()->addAnd(array('datetime','name','version'),
			array('i','s','s'),array($this->i_date,$s_browser,$s_version));
			
		$service_Database	= $this->service_QueryBuilder->getResult();
		if( $service_Database->affected_rows() == 0 ){
			$this->service_QueryBuilder->insert('stats_browser',array('name','amount','datetime','version'),array('s','i' ,'i','s'),
				array($s_browser,1,$this->i_date,$s_version))->getResult();
		}
	}
	...
	public function saveReference($s_reference){
		Memory::type('string',$s_reference);
	...
	public function saveScreenSize($i_width,$i_height){
		Memory::type('int',$i_width);
		Memory::type('int',$i_height);

		$this->service_QueryBuilder->update('stats_screenSizes','amount','l','amount + 1')->getWhere()->addAnd(array('datetime','width','height'),
			array('i','i','i'),array($this->i_date,$i_width,$i_height));
		$service_Database = $this->service_QueryBuilder->getResult();	
	...
	public function saveScreenColors($s_screenColors){
		Memory::type('string',$s_screenColors);
	...
	public function getScreenColors($i_month,$i_limit = -1){
		...
		if( $i_limit > 0 ){	$this->service_QueryBuilder->limit($i_limit); }
		$service_Database = $this->service_QueryBuilder->getResult();			
