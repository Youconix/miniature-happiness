...
class Data_Group extends \core\models\Model{
  ...
  public function __construct(\core\services\QueryBuilder $service_QueryBuilder,\core\services\Security $service_Security){
    ...
    $this->a_validation = array(
      's_name' => array('type'=>'string','required'=>1),
      'i_default' => array('type'=>'enum','set'=>array(0,1)),
      's_description' => array('type'=>'string','required'=>1)
    );
  }
  ...
  public function setDefault($bo_default){
    ...
    if( $bo_default ){ $this->i_default = 1;  } else {  $this->i_default = 0;  }
  }
  ...
  public function save(){
    ...
    $this->service_QueryBuilder->insert('groups', array( 'id', 'name', 'description', 'automatic' ), array( 'i', 's', 's', 's' ), array( $this->i_id, $this->s_name, $this->s_description, $this->i_automatic ));
    ...
    if( $this->i_automatic == 1 ){
      ...
      foreach( $a_users AS $a_user ){
        $i_level = 0;
        if( $a_user[ 'staff' ] == \core\services\Session::ADMIN ) $i_level = 2;
  ...
  public function persist(){
    ...
    $this->service_QueryBuilder->update('groups', array( 'name', 'description', 'automatic' ), array( 's', 's', 's', 'i' ), array( $this->s_name, $this->s_description, $this->i_default, $this->i_id ))->getResult();
  }
  ...
  public function editUser($i_userid, $i_level = 0){
    ...
    if( !in_array($i_level, array( -1, 0, 1, 2 )) ) return;
  ...
  public function addUsersToDefault(){
    ...
    foreach( $a_users AS $i_user ){
      if( array_key_exists($i_user, $a_currentUsers) ) continue;
  ...
  public function inUse(){
    if( is_null($this->i_id) ) return false;
