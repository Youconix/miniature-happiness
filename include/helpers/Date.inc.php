...
class Helper_Date extends Helper {
     ...
     * 
     * @param int   $i_hours    The hours
     * @param int   $i_minutes  The minutes
     * @param int   $i_seconds  The seconds
     * @param int   $i_day      The day from the month
    ...
    public function getTime($i_hours = -1,$i_minutes = -1,$i_seconds = -1,$i_day = -1,$i_month = -1,$i_year = -1){
        if( $i_hours == -1 )    $i_hours = date("H");
        if( $i_minutes == -1 )  $i_minutes = date("i");
        if( $i_seconds == -1 )  $i_seconds = date("s");
        if( $i_month == -1 )    $i_month = date("n");
        if( $i_day == -1 )      $i_day = date("j");
        if( $i_year == -1 )     $i_year = date("Y");
     ...
     * 
     * @param int $i_days		Number of days
     * @param int $i_months		Number of months
     * @param int $i_years		Number of years
     * @param int $i_hours		Number of hours
     * @param int $i_minutes	Number of minutes
     * @param int $i_seconds	Number of seconds
     * @return int	The date
     */
     ...
     * @param int $i_month		The month
     * @param int $i_year		The year
     * @param int $i_hours		The number of hours
     * @param int $i_minutes	The number of minutes
     * @param int $i_seconds	The number of seconds
     * @return boolean	True if the date and time is valid
     */
    ...
    public function validateDate($i_day,$i_month,$i_year){
    	if( !is_int($i_day) || !is_int($i_month) || !is_int($i_year) )	return false;
    	if( $i_month < 1 || $i_month > 12 )		return false;
    	
    	$i_days	= $this->getDaysMonth($i_month, $i_year);
    	if( $i_day < 1 || $i_day > $i_days )	return false;
    ...
    public function getDaysMonth($i_month,$i_year){
    	switch($i_month){
    		...
    		case 2 :
    			if( $i_year % 400 == 0 )	return 29;
    			if( $i_year % 100 == 0 )	return 28;
    			if( $i_year % 4 == 0 )		return 29;
    			return 28;
    	}    	
    }
}
