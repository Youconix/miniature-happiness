 ...
 */
class Helper_Calender extends Helper {
    ...
    public function setYear($i_year){
        if( $i_year > 0 )   $this->i_year   = $i_year;
    }
     ...
     * @param int   $i_day The week start date
     */
    public function setStartDay($i_day){
        if( $i_day >= 0 && $i_day <= 6 )    $this->i_startDayWeek   = $i_day;
    }
    
    /**
     * Sets the callback event
     * @param string $s_event the callback event
     */
    ...
    public function generateCalender(){
        ...
        Memory::services('Template')->headerLink('<script src="{NIV}js/calender.php" type="text/javascript"/>');
