...
class Database_Mysqli implements DAL {
    ...
    public function defaultConnect() {
        ...
        $s_type = $service_XmlSettings->get('settings/SQL/type');
        $this->connection($service_XmlSettings->get('settings/SQL/'.$s_type.'/username'),
                $service_XmlSettings->get('settings/SQL/'.$s_type.'/password'),
                $service_XmlSettings->get('settings/SQL/'.$s_type.'/database'),
                $service_XmlSettings->get('settings/SQL/'.$s_type.'/host'),
                $service_XmlSettings->get('settings/SQL/'.$s_type.'/port'));
    ...
    public function checkLogin($s_username,$s_password,$s_database,$s_host = '127.0.0.1',$i_port = -1){
        ...
        if(file_exists(NIV.'admin/data/settings/settings.xml') && class_exists('Memory') ){
            ...
            if( $s_username == $service_XmlSettings->get('settings/SQL/'.$s_type.'/username') && $s_password == $service_XmlSettings->get('settings/SQL/'.$s_type.'/password') 
                 && $s_database == $service_XmlSettings->get('settings/SQL/'.$s_type.'/database') && $s_host == $service_XmlSettings->get('settings/SQL/'.$s_type.'/host') 
                 && $i_port == $service_XmlSettings->get('settings/SQL/'.$s_type.'/port')){
                return true;
            }
        }
