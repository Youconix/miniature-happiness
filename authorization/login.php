...
class Login extends \includes\BaseLogicClass {
 ...
 protected function index($bo_callback = false){
  ...
  if( $model_Config->isFacebookLogin() ){
   $this->service_Template->setBlock('login',array('image'=>'facebook', 'key'=>'facebook','text'=>$this->service_Language->get('loginFacebook')));
  }
  if( $model_Config->isOpenIDLogin() ){
   $this->service_Template->setBlock('login',array('image'=>'openID', 'key'=>'openID','text'=>$this->service_Language->get('loginOpenID')));
  }
 }
  ...
  * Regular login only
  *
  * @param string $s_notice form notice, optional
  */
