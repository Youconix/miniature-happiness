...
class Registration extends \core\BaseLogicClass {
	...
	private function form(){
		...
		$a_fields	= array('openID','facebook');
		...
		foreach( $a_fields AS $s_login ){
			$this->service_Template->setBlock('openID',array('key'=>$s_login,'image'=>strtolower($s_login), 'text'=>Language::text('registration/registration_'.$s_login) ));
		}
	 ...
	 *
	 * @return boolean	True if all the fields are filled in
	 */
	private function check(){
		$a_fields       = array('nick','email','password');
		$bo_error       = false;
