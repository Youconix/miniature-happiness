...
Modules.prototype.deleteModule	= function(){
	...
	confirmBox.show(languageAdmin.modules_delete_title,languageAdmin.modules_delete.replace('[name]',name));
}
Modules.prototype.deleteModuleCallback	= function(){
	var id = modules.item.data('id');
	
	$.post(this.address,{'command':'delete','id':id},function(){
		general.showModules();
	});
}
Modules.prototype.upgrade	= function(){
	...
	confirmBox.show(languageAdmin.modules_upgrade_title,languageAdmin.modules_upgrade.replace('[name]',name));
}
Modules.prototype.upgradeCallback	= function(){
	var id = modules.item.data('id');
	
	$.post(this.address,{'command':'upgrade','id':id},function(){
		general.showModules();
	});
}
Modules.prototype.install	= function(){
	...
	confirmBox.show(languageAdmin.modules_install_title,languageAdmin.modules_install.replace('[name]',name));
}
Modules.prototype.installCallback	= function(){
	var name = modules.item.data('name');
	
	$.post(this.address,{'command':'install','name':name},function(){
		general.showModules();
	});
}
