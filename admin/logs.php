...
class Logs extends AdminLogicClass {
	...
	public function __construct() {
		...
		if( isset($this->get['command']) ){
			...
			else if ($this->get['command'] == 'view') {
				$this->view();
			} else if ($this->get['command'] == 'download') {
				$this->download();
			}
		}
	...
	private function index() {
		...
		foreach ($a_types AS $s_type) {
			$this->service_Template->setBlock('log',array('logtype'=>$s_type));
	 ...
	 *
	 * @param   string  $s_logName  The name of the log
