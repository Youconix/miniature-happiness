...
Validation.prototype.bindNames = function (names) {
  var i;
  for (i in names) {
    var type = $('[name="' + names[i] + '"]').prop('type');

    if ((!type) || (arraySearch(validation.skip, type) === -1)) {
      $('[name="' + names[i] + '"]').on("blur", function () {
	var item = $(this);
...
Validation.prototype.trigger = function(item,error_type){
  $(item).removeClass('valid').addClass('invalid');
  validation.errorMessage($(item),error_type);
}
