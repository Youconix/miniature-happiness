...
class testHeaders extends GeneralTest{
  ...
  public function contentType(){
  	...
  	$this->assertEquals(array('Content-Type',$s_contentType),$a_headers['Content-Type']);
  }
  ...
  public function modified(){
  	$i_modified = time();
  	$expected = array('Content-Type' => array('Content-Type','text/html'), 'Last-Modified' => array('Last-Modified',gmdate('D, d M Y H:i:s', $i_modified).' GMT'));
  ...
  public function cache(){
  	...
  	$expected = array('Expires',gmdate('D, d M Y H:i:s',(time() + $i_cache)).' GMT');
  ...
  public function cacheDisabled(){
  	$expected = array(
  		'Content-Type' => array('Content-Type','text/html'),
  		array('Expires','Thu, 01-Jan-70 00:00:01 GMT'),
  		'Last-Modified' => array('Last-Modified',gmdate('D, d M Y H:i:s') . ' GMT'),
  ...
  public function contentLength(){
  	...
  	$expected = array('Content-Length',$i_length);
  ...
  public function setHeader(){
  	...
  	$expected = array('Content-Type'=>array('Content-Type','text/html'),array($s_key,$s_content));
  ...
  public function http304(){
  	$expected = array('HTTP/1.1','304 Not Modified');
  ...
  public function http400(){
  	$expected = array('HTTP/1.1','400 Bad Request');
  ...
  public function http401(){
  	$expected = array('HTTP/1.1','401 Unauthorized');
  ...
  public function http403(){
  	$expected = array('HTTP/1.1','403 Forbidden');
  ...
  public function http404(){
  	$expected = array('HTTP/1.1','404 Not Found');
  ...
  public function http503(){
  	$expected = array('HTTP/1.1','503 Service Unavailable');
