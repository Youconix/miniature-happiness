...
class AdminUserEdit extends AdminUserView {
 ...
 protected function setGroups(){
  ...
  foreach( $a_groups AS $i_id => $i_level ){
   ...
   $a_data = array('name'=>$obj_group->getName(),'level'=> $this->service_Language->get('system/rights/level_'.$i_level),
     'levelNr' => $i_level,'id'=> $obj_group->getID());
  ...
  foreach( $a_groups AS $obj_group ){
   if( in_array( $obj_group->getID(),$a_currentGroups) ){ continue; }
   
   
   $this->service_Template->setBlock('newGroup',array('value'=>$obj_group->getID(),'text'=>$obj_group->getName()));
  }
  
  for( $i=0; $i<=2; $i++ ){
   $this->service_Template->setBlock('newLevel',array('value'=>$i,'text'=>$this->service_Language->get('system/rights/level_'.$i)));
  }
 }
}