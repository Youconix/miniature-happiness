...
<span style="float: right;">
  ...
  <br /> <a
		href="make_subset.php?fontfile=<?php echo $fontfile; ?>&amp;name=<?php echo urlencode($font->getFontSubfamilyID()); ?>">Make
			a subset of this font</a>
	</span>
    ...
    foreach ($font->getTable() as $entry) {
        ...
        ?>
      <li><a <?php if (!$data) { ?> style="color: #ccc;" <?php } ?>
  ...
  <?php
    ...
    foreach ($font->getTable() as $table) {
        ...
        ?>
    <div
			id="tabs-<?php echo preg_replace("/[^a-z0-9]/i", "_", $tag); ?>">
      <?php
        if ($data) {
