...
class Database_Mysqli_binded
{
     ...
     *            to excequte
     * @param array $a_types
     *            types : i (int) ,d (double) ,s (string) or b (blob), also accepts a single value
     * @param array $a_values
     *            values, also accepts a single value
     * @param object $link
     *            link
    ...
    public function queryBindedProcess($s_query, $a_types, $a_values, $link)
    {
        ...
        for ($i = 0; $i < $i_num; $i ++) {
            ...
            if (! is_string($s_type) || ! in_array($s_type, array(
                'i',
                'd',
                's',
                'b'
     ...
     * @return array data-set with named keys sorted on the given key
     * @throws DBException when no SELECT-query was excequeted
     */
